<#macro user_comments pageNo pageSize username isPaginate=false isFooter=false>
    <div class="card">
        <@tag_user_comments username=username pageNo=pageNo pageSize=pageSize>
            <div class="card-header">Questions with ${username} comments</div>
            <#if comments.total == 0>
                <div class="card-body">
                    Nothing
                </div>
            <#else>
                    ...
                    <#list comments.records as comment>
                        <tr>
                            <td>
                                ...
                                created › <a href="/topic/${comment.topicId}">${comment.title!?html}</a>
                            </td>
                        </tr>
                ...
                <#if isFooter>
                    <div class="card-footer">
                        <a href="/user/${username}/comments">${username} more comments&gt;&gt;</a>
                    </div>
                </#if>
            </#if>
        </@tag_user_comments>
